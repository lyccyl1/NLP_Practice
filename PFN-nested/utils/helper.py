...
class collater():
    ...
    def __call__(self, data):
        ...
        rc_head_labels = [item[2] for item in data]
        rc_tail_labels = [item[3] for item in data]
        bert_len = [item[4] for item in data]

        batch_size = len(words)

        max_len = max(bert_len)
        ner_labels = [gen_ner_labels(ners, max_len, self.ner2idx) for ners in ner_labels]
        rc_head_labels = [gen_rc_labels(rcs,max_len, self.rel2idx) for rcs in rc_head_labels]
        rc_tail_labels = [gen_rc_labels(rcs, max_len, self.rel2idx) for rcs in rc_tail_labels]

        ner_labels = torch.stack(ner_labels, dim=2)
        rc_head_labels = torch.stack(rc_head_labels, dim=2)
        rc_tail_labels = torch.stack(rc_tail_labels, dim=2)
        mask = mask_to_tensor(bert_len, batch_size)

        return [words,ner_labels,rc_head_labels,rc_tail_labels,mask]
