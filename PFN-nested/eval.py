...
from model.pfn_from_scratch import *
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for data in tqdm(test_batch):
            ...
            re_label_head = data[2].to(device)
            re_label_tail = data[3].to(device)
            mask = data[-1].to(device)

            ner_pred, re_pred_head, re_pred_tail = model(text, mask)
            entity_num = metric.count_ner_num(ner_pred, ner_label)
            triple_num = metric.count_num(ner_pred, ner_label, re_pred_head, re_pred_tail, re_label_head, re_label_tail)
