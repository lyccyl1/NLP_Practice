...
from model.pfn_from_scratch import PFN
...
def evaluate(test_batch, rel2idx, ner2idx, args, test_or_dev):
    ...
    with torch.no_grad():
        for data in test_batch:
            ...
            loss = BCEloss(ner_pred, ner_label, re_head_pred, re_tail_pred, re_head_label, re_tail_label)
            test_loss += loss

            entity_num = metric.count_ner_num(ner_pred, ner_label)
            triple_num = metric.count_num(ner_pred, ner_label, re_head_pred, re_tail_pred, re_head_label, re_tail_label)
